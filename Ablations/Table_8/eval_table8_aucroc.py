#!/usr/bin/env python3
"""
Evaluate micro frame-level AUC-ROC for Table 8 (Stage 2 + smoothing) score exports.

Default layout under --root (script directory):
  <root>/<DATASET>/video_*.npy     # per-video Stage-1 frame scores + Gaussian smoothing
  <root>/<DATASET>/gt_concat.npy   # concatenated ground truth (0/1)

Alternatively, pass explicit directories:
  --score-dir ./XD-Violence --dataset XD-Violence --gt ./XD-Violence/gt_concat.npy

GT may also come from:
  --gt-root ./Ground_Truth   # expects <gt-root>/<DATASET>/gt_concat.npy
"""
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        description="Evaluate Table 8 Stage-2 + smoothing scores vs ground truth."
    )
    ap.add_argument(
        "--root",
        type=Path,
        default=SCRIPT_DIR,
        help="Table_8 root with one subfolder per dataset (default: script directory)",
    )
    ap.add_argument(
        "--datasets",
        nargs="+",
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Single score directory (use with --dataset and optional --gt)",
    )
    ap.add_argument(
        "--dataset",
        type=str,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Ground-truth .npy path (default: <score-dir>/gt_concat.npy)",
    )
    ap.add_argument(
        "--gt-root",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Output JSON path (default: <root>/table8_aucroc.json)",
    )
    ap.add_argument(
        "--out-csv",
        type=Path,
        default=None,
        help="Output CSV path (default: <root>/table8_aucroc.csv)",
    )
    args = ap.parse_args(argv)

    root = args.root.resolve()
    out_json = args.out_json or (root / "table8_aucroc.json")
    out_csv = args.out_csv or (root / "table8_aucroc.csv")
    ...
    payload = {
        ...
        "description": "Table 8 Stage-2 frame scores + Gaussian smoothing vs gt_concat.npy",
        "results": results,
    }
